...
export const NoDataDisplay = React.memo(({message}) => (
...
export const ChartWrapper = React.memo(({title, hasData, noDataMessage, height = "h-80", action, children, hideChartWhenNoData = false}) => (
...
// User Charts
export const NewUsersByDayChart = React.memo(({data}) => {
    console.log('NewUsersByDayChart data:', data);
    ...
    return (
        ...
        <ResponsiveContainer width="100%" height="100%">
            ...
            >
                ...
                />
                <Legend/>
                <Bar dataKey="count" name="Người dùng mới" fill="#3b82f6"/>
            </BarChart>
        </ResponsiveContainer>
    </ChartWrapper>
    );
...
export const UsersByAuthProviderChart = React.memo(({data}) => {
    console.log('UsersByAuthProviderChart data:', data);
...
export const UserAuthProviderCard = React.memo(({item}) => (
    <div className="bg-white dark:bg-gray-700 rounded-lg shadow p-4 flex items-center">
        <div
            className={`p-3 rounded-full ${item.provider === 'GOOGLE' ? 'bg-red-500' : 'bg-blue-500'} text-white mr-4`}>
            <FontAwesomeIcon icon={item.provider === 'GOOGLE' ? faGoogle : faUser} className="h-5 w-5"/>
        </div>
        <div>
...
// Manga Charts
export const MangaGenreChart = React.memo(({data}) => {
    // Log ra để kiểm tra số lượng thể loại thực tế
    console.log('Tổng số thể loại:', Object.keys(data).length);
    console.log('Dữ liệu thể loại:', data);

    // Map dữ liệu và không giới hạn số lượng, hiển thị tất cả
    ...
    // Tạo một danh sách màu cố định để đảm bảo tính nhất quán
    const COLORS = [
        '#8884d8', '#83a6ed', '#8dd1e1', '#82ca9d', '#a4de6c',
        '#d0ed57', '#ffc658', '#ff8042', '#ff6361', '#bc5090',
        '#58508d', '#003f5c', '#665191', '#a05195', '#d45087',
        '#f95d6a', '#ff7c43', '#ffa600', '#2f4b7c', '#f95d6a',
        '#ff7c43', '#ffa600', '#003f5c', '#7a5195'
    ];
            ...
            <div className="h-full flex flex-col">
                <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                        ...
                        <Tooltip
                            formatter={(value, name) => [`${value} truyện`, name]}
                        />
                    </PieChart>
                </ResponsiveContainer>
            </div>
...
export const MangaStatusChart = React.memo(({data}) => {
    ...
    return (
        <ChartWrapper
            ...
            height="h-[450px]" // Tăng chiều cao để biểu đồ hiển thị lớn hơn
        >
            <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                    <Pie
                        ...
                        cy="50%"
                        labelLine={true}
                        outerRadius={150} // Tăng từ 80 lên 150 để biểu đồ lớn hơn
                        fill="#8884d8"
                        dataKey="value"
                        nameKey="name"
                        label={({name, percent}) => `${name}: ${(percent * 100).toFixed(0)}%`}
...
export const MostViewedMangaChart = React.memo(({data}) => (
    <ChartWrapper
...
// View Charts
export const ViewsByDayChart = React.memo(({data, startDate, endDate, setStartDate, setEndDate}) => {
...
export const ViewsByMangaChart = React.memo(({data, limit, setLimit}) => {
    const limitSelector = (
        <div className="flex items-center space-x-2">
            <FontAwesomeIcon icon={faChartLine} className="text-gray-500 dark:text-gray-400"/>
            <select
