...
export const useChapterManagement = (initialItemsPerPage = 10) => {
  ...
  const fetchChapters = useCallback(async () => {
    ...
    try {
      ...
      const response = await chapterService.getAllChapters(currentPage - 1, itemsPerPage, mangaId);
  ...
  // Tìm kiếm manga
  const searchMangas = useCallback(async (term) => {
    if (!term) {
  ...
  const createChapter = async (formData) => {
    ...
    try {
      const response = await chapterService.createChapter(formData);
      if (response) {
  ...
  const updateChapter = async (chapterId, formData) => {
    setIsSubmitting(true);
    try {
      const response = await chapterService.updateChapter(chapterId, formData);
      if (response) {
